...
#pragma omp parallel reduction(+ \
                               : validos, invalidos, quantidade_p)
    {
        ...
        fscanf(read_file, "%ld", &s);
        fscanf(read_file, "%ld", &f);
        fscanf(read_file, "%ld", &e);
        ...
        char *line;
        line = (char *) malloc(sizeof(int) * 10);
        size_t size = sizeof(int);
        int total_read = start_position;
        while (fread_unlocked(line, 10, 1, read_file)!= EOF)
        {
            voto = atoi(line);
            size = strlen(line);
            total_read+=size;
            if (total_read > end_position && thread_num<(threads))
	    {
                break;
            }
            if (voto >= 10)
